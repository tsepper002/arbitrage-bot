...
class XtSocket:
    ...
    def runWebSockPl(self, key):
        while True:
            try:
                wss = 'wss://ws.xt.com/ws'  # URL для XT
                ...
                print(f'Установлено соединение WebSocket для ключа {key} с XT')
                ...
                print(f'Разрыв соединения WebSocket для ключа {key} с XT')
                self.ticker_socket[key]['status'] = 'off'
                time.sleep(10)

    def start(self):
        response = requests.get("https://api.xt.com/v2/market/tickers")
        data = response.json()

        i = 1
        listPair = {}
        for elem in data['data']['tickers']:
